...
class Particle {
    ...
    draw() {
        ...
        // Draw a little star/sparkle shape
        ctx.beginPath();
...
class Boss extends Entity {
    constructor(x, y, type, minX, maxX) {
        ...
        this.type = type; 
        // BALANCING: Reduced health for Boss 2 (The Jumper)
...
function updatePeekMechanic() {
    ...
    if (peekState === PEEK_STATE.SAFE) {
        ...
        // DETECTION: If player moves OR shoots while child is peeking
        const isMoving = Math.abs(player.vx) > 0.1 || Math.abs(player.vy) > 0.1;
        const isShooting = keys.z; // This ensures Z key counts as movement
...
function startGame() { document.getElementById('start-screen').style.display = 'none'; gameRunning = true; loadLevel(1); update(); }
